...
//return ok and error string
func SavePhoto(userPhoto *UserPhoto, userPhotoTable string, awsDbClient *dynamodb.DynamoDB, anlogger *syslog.Logger, lc *lambdacontext.LambdaContext) (bool, string) {
	anlogger.Debugf(lc, "common_action.go : save photo %v for userId [%s]", userPhoto, userPhoto.UserId)
	input :=
		&dynamodb.UpdateItemInput{
			ExpressionAttributeNames: map[string]*string{
				"#photoType":   aws.String(PhotoTypeColumnName),
				"#photoBucket": aws.String(PhotoBucketColumnName),
				"#photoKey":    aws.String(PhotoKeyColumnName),
				"#photoSize":   aws.String(PhotoSizeColumnName),
				"#updatedAt":   aws.String(UpdatedTimeColumnName),
			},
			...
			ConditionExpression: aws.String(fmt.Sprintf("attribute_not_exists(%s)", PhotoDeletedAtColumnName)),
	...
	if userPhoto.PhotoSourceUri != "" {
		input.ExpressionAttributeNames["#photoUri"] = aws.String(PhotoSourceUriColumnName)
