...
//return ok and error string
func MarkPhotoAsHiddenInModerationProcess(userId, photoId, tableName string, awsDbClient *dynamodb.DynamoDB, anlogger *commons.Logger, lc *lambdacontext.LambdaContext) (bool, string) {
	anlogger.Debugf(lc, "common_action.go : mark photoId [%s] as hidden during moderation for userId [%s]", photoId, userId)
	input :=
		&dynamodb.UpdateItemInput{
			ExpressionAttributeNames: map[string]*string{
				"#deletedAt": aws.String(commons.PhotoDeletedAtColumnName),
			...
			TableName:        aws.String(tableName),
			UpdateExpression: aws.String("SET #deletedAt = :deletedAtV, #hiddenAt = :hiddenAtV"),
		}
	_, err := awsDbClient.UpdateItem(input)
	if err != nil {
		anlogger.Errorf(lc, "common_action.go : error while mark photo as hidden during moderation photoId [%s] for userId [%s] : %v", photoId, userId, err)
		return false, commons.InternalServerError
	}
